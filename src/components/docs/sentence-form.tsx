...
const SentenceForm = ({
    ...
    const updateFormData = (fieldName: string, value: string | number | boolean) => {
    ...
    `;
        ...
        if (config.type === "select") {
            return (
                ...
                >
                    {Icon && (
                        <Icon className={`w-4 h-4 mr-2 ${isCompleted ? "text-white" : "text-white/90"}`} />
                    )}
        ...
        return (
            ...
            >
                ...
                <input
                    ...
                    style={{ width: `${Math.max((config.placeholder ?? '').length, String(currentValue).length, 8)}ch` }}
                />
                {isCompleted && (
                    <motion.div
                        initial={{ scale: 0, opacity: 0 }}
